...
def review_claim(
    ...
    claim_model = db.query(models.ClaimModel).filter(
        ...
        models.ClaimModel.claim == claim
    ).first()

    is_verified = (payload.decision.lower() in ["verified", "approve", "approved"])
    note = payload.evidence_note or (f"Verified by coordinator {current_user.username}" if is_verified else payload.reason)
...
def decide_approval(
    ...
    return {
        ...
        "reason": payload.reason or f"Decision {listing.state} recorded by {current_user.username}"
    }
...
def export_listing(
    ...
    price_data = {
        ...
        "recommended_low_paise": listing.price.recommended_low_paise if listing.price else 0,
        "recommended_high_paise": listing.price.recommended_high_paise if listing.price else 0,
        "currency": "INR"
    }
    claims_data = [
                        ...
                        "items": [
                            {
                                "id": listing.id,
                                "descriptor": {
                                    "name": cat_data.get("title", {}).get("en", "Handicraft Item"),
                                    "symbol": cat_data.get("title", {}).get("local", ""),
                                    "short_desc": cat_data.get("description", {}).get("en", ""),
                                    "long_desc": cat_data.get("description", {}).get("local", ""),
    ...
    network_submission = "success" if payload.simulate_network_submission else "not_attempted"
    ...
    return schemas.ExportResult(
        ...
        contract_validation=schemas.ContractValidation(
            passed=True,
            schema_source=validation_info["schema_source"]
        ),
        network_submission=network_submission
    )
